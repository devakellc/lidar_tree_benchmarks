...
# Classical arms — all three sites
...
The classical arms (AMS3D, lmfauto, multichm, ptrees, CHM-VWF) have been run on
all three NEON sites across the full density ladder. Regenerate with
`compare_model_sites.R` (see Regenerate block above); the section below will be
updated with the results table and density curves.

**Density context for SJER and TEAK.** SJER (open oak savanna) and TEAK (dense
red fir) native clouds are already ~4–6 first-return/m² — within AMS3D's
comfort zone of 4–20 pts/m² (all-return) — so the classical arms are expected
to perform within their tested envelope even at native density. The three-site
structure gradient mirrors the CHM-VWF sweep in `compare_sites.R`: SJER open
structure yields higher recall at low density (fewer occlusions), TEAK's dense
closed canopy suppresses understory recall across all arms and rungs.

*Results table and density curves pending the SJER/TEAK sweep runs.*

<!-- generated by scripts/compare_model_sites.R; insert model_cross_site_summary.csv table here -->
...
- **Single site for deep arms; partial Li 2012.** The GPU arms (TreeisoNet, FF3D)
  and Li 2012 are SOAP-only; a full-ladder Li 2012 remains deferred. The
  classical arms (AMS3D, lidRplugins) extend to SJER and TEAK — see the
  cross-site section below. The native-only Li 2012 here answers the
  dense-input sub-canopy question, not its density response.
